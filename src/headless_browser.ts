...
export class HeadlessBrowser {
  ...
  constructor(urlToVisit: string) {
    ...
    this.browser_process = Deno.run({
        cmd: [chromePath, "--headless","--virtual-time-budget=10000", "--repl", urlToVisit],
        stdin: "piped",
        stdout: "piped",
        stderr: "piped"
      })
  ...
  public async click (selector: string): Promise<void> {
    if (selector.indexOf("href") !== -1) {
      throw new Error("Clicking links is not supported. You can check a link works by getting the `href` attribute")
    }
